...
window.onload = function () {
    if (localStorage.getItem("Patientenliste")) {
        PatinetenListe = JSON.parse(localStorage.getItem("Patientenliste"));
        fillPatientList();

        alert("Daten erfolreich aus dem LocalStorage geladen!");

    } else {
        alert("Keine Daten gefunden. Generiere Testdaten.");
        
        var tetsfjl = [];
        var Falltest = new Fall("einfall");
        Falltest.fallDatum = new Date("2020-02-02");
        tetsfjl.push(Falltest);
        Falltest = new Fall("2fall");
        Falltest.fallDatum = new Date(("2023-03-03"));
        tetsfjl.push(Falltest);
        Falltest = new Fall("3fall");
        tetsfjl.push(Falltest);




        var patient = new Patient("Erster Patient", tetsfjl);
        PatinetenListe.push(patient);

        var patient = new Patient("Zweiter Patient", []);
        PatinetenListe.push(patient);
        var patient = new Patient("Dritter Patient", []);
        PatinetenListe.push(patient);
        //alert(JSON.stringify(PatinetenListe));
    }
}
